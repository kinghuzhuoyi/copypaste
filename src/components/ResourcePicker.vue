...
const entryLabel = computed(() => props.entryType === 'COMPONENT' ? '组件' : '规则')
...
async function loadResources() {
  ...
  try {
    ...
    resourceMap.value = Object.fromEntries(filteredGroups.value.flatMap((group) => group.items.map((item) => [item.resourceId, item])))
...
watch(() => [props.source.versionId, props.entryType], loadResources, { immediate: true })
...
<template>
  <section class="picker">
    <div class="picker-left">
      <div class="toolbar">
        <div>
          <div class="panel-title">选择{{ entryLabel }}</div>
      ...
      <div class="pick-scroll" v-loading="loading">
        <el-empty v-if="!loading && !hasResources" :description="`当前来源版本暂无可拷贝${entryLabel}`" :image-size="78" />
    ...
    <aside class="picker-right">
      <div class="panel-title">已选 {{ selectedResources.length }} 个{{ entryLabel }}</div>
